...
\begin{adjustbox}{center}
            \begin{tikzpicture}[level distance=7mm, sibling distance=6mm,
                ...
                \draw[dashed] (graduation Daniel u) to node[auto] {A} (Daniel u);
                \end{scope}
                \onslide<2->{
                  ...
                  \draw[dashed] (Daniel siyem et halimudim d) to[out=-15,in=-150] node[above] {A} (Daniel d);
                  \end{scope}
                }\onslide<3>{
...
consisting of: stack, buffer and constructed graph \cite{hershcovich2017a}.
...
Shared task \cite{hershcovich2019shared}.
\begin{itemize}
\item Data: UCCA for English, French, German.
\item Tracks: $\{$in-domain/out-of-domain$\} \times \{$open/closed$\}$.
\item Baseline: TUPA.
...
\end{dependency}
    ...
    Multi-task TUPA model \cite{hershcovich2018multitask}
...
\begin{frame}
  ...
  Shared task \cite{Oep:Abe:Haj:19}.
\begin{itemize}
...
\begin{tikzpicture}
    ...
    scatter,only marks,mark size=4,scatter src=explicit symbolic,
...
\begin{frame}[fragile]
  \frametitle{Composition-Based Approaches}
  \begin{center}
        \scalebox{0.7}{
\begin{tikzpicture}
    \begin{axis}[
    ...
    tickwidth=0pt,
    axis background/.style={fill=cyan!10},
    scatter/classes={1={mark=square,red},
                   2={mark=triangle,gray,draw opacity=.3},
                   3={mark=o,gray,draw opacity=.3},
                   4={mark=x,gray,draw opacity=0}
                  },
    scatter,only marks,mark size=5,scatter src=explicit symbolic,
    ]
    \addplot table[x=a,y=Overall,meta=shape]{\approaches};
    \addplot table[x=b,y=DM,meta=shape]{\approaches};
    \addplot table[x=c,y=PSD,meta=shape]{\approaches};
...
\end{tikzpicture}                
      ...
      \begin{itemize}
        \item Explicitly modeling the derivation process.
        \item A parser evaluates a derivation licensed by a symbolic system. 
      \end{itemize}
  \end{center}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Factorization-Based Approaches}
  \begin{center}
      \begin{center}
        \scalebox{0.7}{
\begin{tikzpicture}
    ...
    tickwidth=0pt,
    axis background/.style={fill=cyan!10},
    scatter/classes={1={mark=square,gray,draw opacity=.3},
...
\end{tikzpicture}        
      ...
      \end{center}
      \begin{itemize}
        \item Inspired by graph-based dependency parsers.
        \item Explicitly modeling the target structure.
        \item A parser evaluates factors of a candidate graph.
      \end{itemize}
  \end{center}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Transition-Based Approaches}
  \begin{center}
      \begin{center}
        \scalebox{0.7}{
\begin{tikzpicture}
...
\end{tikzpicture}            
      ...
      \end{center}
      \begin{itemize}
        \item Inspired by transition-based dependency parsers.
        \item Incremental (left-to-right, word-by-word).
        \item Partial parse constrains subsequent actions.
        \item Greedy/beam search to get a parse.
      \end{itemize}
  \end{center}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Score Distributions: Zoom In}
  \begin{center}\footnotesize
\begin{tikzpicture}
    \begin{axis}[
    enlarge x limits={abs=7mm},
...
Evaluate by matching edges \cite{hershcovich2019content}.
