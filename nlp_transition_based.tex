...
\begin{frame}
    ...
    Instead of sparse binary features, uses dense embedding features.
...
\begin{frame}
    ...
    Add bidirectional LSTM RNN to get token representations.
	...
	\foreach \i/\word in {1/{John},3/{looked},5/{at},7/{Mary}} {
        ...
        \path (x\i) edge node[left]{\textit{embedding}} (h\i);
	    \node[main, fill=white!100] (i\i) at (\i.5,.8) {LSTM};
        \path (x\i) edge [bend right] node[right]{\textit{embedding}} (i\i);
