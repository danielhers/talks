...
\begin{frame}
    ...
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{stackrnn.png}
\end{frame}
...
\begin{frame}
	...
	\uncover<5->{
        \node[main, fill=white!100] (mlp) at (4,4.6) {\textsc{mlp}};
		\foreach \i in {1,3} {
	        \path (l\i) edge (mlp);
	        \path (k\i) edge (mlp);
	    }
...
\begin{frame}
    ...
    MLP \cite{chen2014fast} & 91.8 & 89.6 \\
    Stack LSTM \cite{dyer2015transition} & 92.4 & 90.0 \\
...
%\begin{frame}
%  \frametitle{Label bias and global normalization}
%\end{frame}
...
\begin{frame}
  ...
  The \textit{projectivity} property does not hold:
  \[
