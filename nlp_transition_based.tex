...
\begin{frame}
  ...
  where
  \begin{itemize}
    \item $\Sigma \subseteq V_x$ is the stack of partially processed items.
    \item $B \subseteq V_x$ is the buffer of remaining input tokens.
...
\begin{frame}
  ...
  \begin{tabular}{ll}
    ...
    & $(\Sigma, \; x \;|\; B, \; A) \Rightarrow (\Sigma \;|\; x, \; B, \; A)$ \\
    \hline
    $\textsc{left-arc}_X$ & create arc $s_0 \to s_1$ with label $X \in \mathcal{L}$ and remove $s_1$: \\
    & $(\Sigma \;|\; s_1,s_0, \; B, \; A) \Rightarrow (\Sigma \;|\; s_0, \; B, \; A \cup \{(s_0,X,s_1)\})$ \\
    & Condition: $s1\neq0$ \\
    \hline
    $\textsc{right-arc}_X$ & create arc $s_1 \to s_0$ with label $X \in \mathcal{L}$ and remove $s_0$: \\
    & $(\Sigma \;|\; s_1,s_0, \; B, \; A) \Rightarrow (\Sigma \;|\; s_1, \; B, \; A \cup \{(s_1,X,s_0)\})$  
  \end{tabular}
...
\begin{frame}
  \frametitle{Arc-eager transition system}
